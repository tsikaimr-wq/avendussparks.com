...
(function () {
    ...
    class MarketEngine {
        ...
        getFrozenKnownPrice(symbol, stock = null) {
            ...
            const subscriptionPrice = this.toFiniteValue(resolvedStock?.subscription_price);
            if (subscriptionPrice !== null && subscriptionPrice > 0) return subscriptionPrice;
                ...
                try {
                    ...
                    if (data && data.length > 0) {
                        ...
                        this.dbInsStocks.forEach(s => {
                            if (allowVisualUpdate && s.market_symbol && (s.price === 0 || !s.isCached)) {
                ...
                if (window.DB && typeof window.DB.getActiveProductsByType === 'function') {
                    ...
                    try {
                        const loadProductRows = async (type) => {
                            let rows = await window.DB.getActiveProductsByType(type);
                            ...
                            if (!Array.isArray(turboRows) || turboRows.length === 0) return Array.isArray(rows) ? rows : [];
        ...
        async fetchMarketPrice(symbol) {
            ...
            try {
                ...
                if (this.applyLockedPrice(stockMatch || symbol)) {
                    return this.getLockedPriceValue(stockMatch || symbol);
                }
                if (!this.isWithinPriceUpdateWindow()) {
                    return this.getFrozenKnownPrice(symbol, stockMatch);
                }
